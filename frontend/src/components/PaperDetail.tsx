...
import React, { useState, useEffect, useRef } from 'react';
import {
  X, Calendar, Users, ExternalLink, BookOpen, TrendingUp,
  FileText, Pencil, Lightbulb, AlertTriangle, Sparkles, Save,
} from 'lucide-react';
...
/* ───────────── tiny re-usables ───────────── */
const InfoBit: React.FC<{ icon: React.ReactNode }> = ({ icon, children }) => (
  <span className="flex items-center space-x-1">{icon}{children}</span>
);
const InfoRow: React.FC<{ icon: React.ReactNode; label: string }> = ({
  icon, label, children,
}) => (
...
const EditableInline: React.FC<{
  ...
  const [editing, setEditing] = useState(!value);
  const [tmp, setTmp] = useState(value ?? '');
  const commit = () => { onSave(tmp.trim()); setEditing(false); };
  if (editing)
    return (
      <input
        autoFocus
...
const EditableBulletList: React.FC<{
  ...
  const commit = () => {
    const next = [...items];
    if (editIdx === -1) { if (tmp.trim()) next.push(tmp.trim()); }
    else if (editIdx !== null) { next[editIdx] = tmp.trim(); }
    onSave(next);
    setEditIdx(null); setTmp('');
  };
...
const PaperDetail: React.FC<Props> = ({ paper, isOpen, onClose, onUpdate }) => {
  ...
  const askAI = () => {
    ...
    navigate(`/chat?title=${encodeURIComponent(title)}&chatid=${chatId}`);
  };
    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <div className="flex flex-wrap gap-x-6 gap-y-2 text-sm text-gray-600 mt-1">
              ...
              <InfoBit icon={<TrendingUp className="h-4 w-4" />}>
                <EditableInline
                  ...
                  placeholder="0"
                  onSave={(v) =>
                    updateField('citationCount')(Number(v.replace(/\D/g, '')) || 0)
                  }
                />
                <span className="ml-1">citations</span>
              </InfoBit>
        ...
        <div className="p-6 pt-5 space-y-8">
              ...
              {draft.keywords?.length ? (
                ...
                <div className="mt-2">
                  <EditableInline
                    value=""
                    placeholder="Add keywords"
                    onSave={(v) => updateField('keywords')(trimArr(v.split(/[,;]/)))}
                  />
                </div>
              )}
            </div>
          ...
          {/* limitations */}
          <Section icon={<AlertTriangle className="h-5 w-5 text-red-500" />} title="Study Limitations">
            <EditableBulletList
              items={draft.limitations ?? []}
              placeholder="Add limitation"
              onSave={(v) => updateField('limitations')(trimArr(v))}
          ...
          {/* replication */}
          <Section icon={<TrendingUp className="h-5 w-5 text-purple-600" />} title="Replication Suggestions">
            <EditableBulletList
              items={draft.replicationSuggestions ?? []}
              placeholder="Add suggestion"
              onSave={(v) => updateField('replicationSuggestions')(trimArr(v))}
            />
          </Section>
          ...
          <div className="flex flex-wrap justify-end gap-4 pt-2">
            <button
              onClick={askAI}
              className="inline-flex items-center gap-2 px-5 py-2 border border-blue-600
